...
async def clean_tables(async_session_test):
    """Очистка таблицы перед запуском тестов"""
...
async def client() -> Generator[TestClient, Any, None]:
    """
    Создание FastAPI TestClient, для переопределения в функции `get_db` на тестовую
    для получения `db_session` которая подставляется в роуты для тестирования
    """
    ...
    async def create_user_in_database(
        ...
        async with asyncpg_pool.acquire() as connection:
            return await connection.execute(
                ...
                user_id, name, surname, email, is_active, hashed_password, roles
            )
